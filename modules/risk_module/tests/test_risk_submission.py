...
class TestRiskSubmission(RiskModuleTestCase):
    ...
    def test_rejecting_approved_submission_releases_master_assignment(self):
        """Rechazar una habilitacion aprobada libera el vehiculo y el conductor,
        permitiendo aprobar despues una nueva con otro conductor (antes el enlace
        maestro quedaba bloqueado para siempre)."""
        first = self.make_submission(
            owner=self.portal_user,
            state="documents_review",
            plate="REL123",
            driver_document_number="44441111",
        )
        self.approve_document(self.make_document(first))
        first.action_confirm_approval("Habilitacion inicial")
        vehicle = first.vehicle_id
        driver = first.driver_id
        self.assertEqual(vehicle.current_driver_id, driver)
        self.assertEqual(vehicle.status, "enabled")
        self.assertEqual(driver.current_vehicle_id, vehicle)

        first.action_confirm_rejection("Cambio de conductor")

        self.assertFalse(vehicle.current_driver_id)
        self.assertEqual(vehicle.status, "available")
        self.assertFalse(driver.current_vehicle_id)

        second = self.make_submission(
            owner=self.other_portal_user,
            state="documents_review",
            plate="REL123",
            driver_document_number="44442222",
        )
        self.approve_document(self.make_document(second))
        second.action_confirm_approval("Reasignacion de conductor")

        self.assertEqual(second.state, "approved")
        self.assertEqual(second.vehicle_id.current_driver_id, second.driver_id)
        self.assertEqual(second.driver_id.document_number, "44442222")

    def test_release_keeps_manually_blocked_vehicle_blocked(self):
        """Liberar la asignacion no debe reabrir un vehiculo bloqueado a mano."""
        first = self.make_submission(
            owner=self.portal_user,
            state="documents_review",
            plate="BLK123",
            driver_document_number="55551111",
        )
        self.approve_document(self.make_document(first))
        first.action_confirm_approval("Habilitacion inicial")
        vehicle = first.vehicle_id
        vehicle.write({"status": "blocked"})

        first.action_confirm_rejection("Bloqueo manual")

        self.assertFalse(vehicle.current_driver_id)
        self.assertEqual(vehicle.status, "blocked")

    def test_release_does_not_clobber_a_newer_assignment(self):
        """Si el vehiculo ya fue reasignado a otro conductor, rechazar la
        solicitud vieja no debe soltar el enlace nuevo."""
        first = self.make_submission(
            owner=self.portal_user,
            state="documents_review",
            plate="GRD123",
            driver_document_number="66661111",
        )
        self.approve_document(self.make_document(first))
        first.action_confirm_approval("Habilitacion inicial")
        vehicle = first.vehicle_id
        old_driver = first.driver_id

        new_driver = self.env["risk.driver"].create(
            {"name": "Conductor Nuevo", "document_number": "66662222"}
        )
        vehicle.write({"current_driver_id": new_driver.id})

        first.action_confirm_rejection("Cambio de conductor")

        self.assertEqual(vehicle.current_driver_id, new_driver)
        self.assertFalse(old_driver.current_vehicle_id)

    def test_approval_closes_obsolete_vehicle_owner_links(self):
        """Aprobar define la foto completa de propietarios: las relaciones
        activas previas que no declara la solicitud se cierran."""
        vehicle = self.env["risk.vehicle"].create(
            {"plate": "OWN123", "status": "available"}
        )
        stale_owner = self.env["risk.owner"].create(
            {
                "name": "Propietario Viejo",
                "document_type": "cc",
                "document_number": "9999999",
            }
        )
        stale_link = self.env["risk.vehicle.owner"].create(
            {"vehicle_id": vehicle.id, "owner_id": stale_owner.id, "role": "owner"}
        )
        self.assertTrue(stale_link.active)

        submission = self.make_submission(
            owner=self.portal_user, state="documents_review", plate="OWN123"
        )
        self.approve_document(self.make_document(submission))
        submission.action_confirm_approval("Aprobada")

        self.assertFalse(stale_link.active)
        self.assertTrue(stale_link.date_to)
        # El enlace propio de la solicitud se conserva activo.
        self.assertTrue(submission.vehicle_owner_link_id.active)
        active_links = self.env["risk.vehicle.owner"].search(
            [("vehicle_id", "=", vehicle.id), ("active", "=", True)]
        )
        self.assertEqual(active_links, submission.vehicle_owner_link_id)
